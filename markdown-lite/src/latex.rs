...
pub fn inline_latex_with_options(md: &str, out: &mut String, options: &mut dyn Options) {
    LaTeXConverter {
        tokenizer: MarkdownTokenizer::new_inline(md),
        states: SmallVec::new(),
        out,
...
impl LaTeXConverter<'_> {
            ...
            match t {
                    ...
                    #[cfg(feature = "frontmatter")]
                    {
                        if let Some(fm) = &mut self.options {
                            match serde_yml::from_str::<'_, serde_value::Value>(s) {
                                ...
                                Err(e) => {
                                    let _ = write!(self.out, "\\errmessage{{{e:#?}}}");
                                }
                            }
                        }
                    }
                ...
                MarkdownToken::InlineCustom { label, args, .. } => {
                        ...
                        && opt.inline_custom(label, args, self.out)
                    {
                        ...
                        let _ = write!(self.out, "@[{label}]({args})");
                    }
                }
                ...
                MarkdownToken::InlineCode(s, _) => {
                    close!(inline);
                    let _ = write!(self.out, "\\lstinline`{s}`");
                }
                ...
                MarkdownToken::UnnumberedListItem(indent, ()) => {
                    ...
                    match self.states.last() {
                            ...
                            if *i < indent =>
                        {
                            ...
                            let _ = self.out.write_str("\\begin{itemize}\n\\item ");
                        }
    ...
    fn close(out: &mut String, state: ConverterState, options: &mut Option<&mut dyn Options>) {
        ...
        match state {
            ...
            ConverterState::Paragraph => out.push_str("\n\n"),
            ConverterState::Strikethrough => out.push('}'),
            ConverterState::Highlight => out.push('}'),
            ConverterState::Superscript => out.push_str("}}$"),
            ConverterState::Subscript => out.push_str("}}$"),
            ConverterState::Heading(lvl) => out.push('}'),
            ConverterState::UnorderedList(_) => out.push_str("\n\\end{itemize}\n"),
            ConverterState::OrderedList(_) => out.push_str("\n\\end{enumerate}\n"),
            ConverterState::InLink => out.push('}'),
